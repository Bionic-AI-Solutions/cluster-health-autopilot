...
Companion to `changelog-lint.sh` (which checks heading format): every released `## [x.y.z]` heading except the topmost (the release in flight) must have a matching `vx.y.z` tag (or chart-releaser `cluster-health-autopilot-x.y.z` tag — the 1.8.x line shipped chart-only cuts under that form), and `[Unreleased]` content may not present a version as already shipped (no dated `[x.y.z] — YYYY-MM-DD` references, no version-numbered headings). Catches the claimed-but-never-tagged release class. Runs in the lint CI job next to changelog-lint; the job's checkout now fetches tags.
